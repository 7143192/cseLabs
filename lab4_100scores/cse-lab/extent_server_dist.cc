...
int extent_server_dist::get(extent_protocol::extentid_t id, std::string &buf) {
    ...
    l->new_command(cmd, term, index);
    //printf("end of getting file %d", id);
    std::unique_lock<std::mutex> lock(cmd.res->mtx);
    //cmd.res->done = false;
    //while(!(cmd->res->done)) {
        //std::this_thread::sleep_for(std::chrono::milliseconds(10));
...
int extent_server_dist::remove(extent_protocol::extentid_t id, int &) {
    // Lab3: your code here
    //printf("begin to remove a file %lld on the extent_server_dist\n", id); 
    chfs_command_raft cmd = chfs_command_raft();
    //cmd.res = std::make_shared<chfs_command_raft::result>();
    //cmd.res->done = false;
    ...
    chfs_raft* l = leader();
    int term, index;
    //printf("begin to remove the file %lld\n", id);
    l->new_command(cmd, term, index);
    std::unique_lock<std::mutex> lock(cmd.res->mtx);
    //cmd.res->done = false;
    //while(!(cmd->res->done)) {
    ...
    //printf("end to remove the file %lld\n", id);
