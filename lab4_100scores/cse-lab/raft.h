...
int raft<state_machine, command>::request_vote(request_vote_args args, request_vote_reply &reply) {
    ...
    std::unique_lock<std::mutex> lock(mtx);//use the global lock to protext the section.
...
int raft<state_machine, command>::append_entries(append_entries_args<command> arg, append_entries_reply &reply) {
    ...
    if(arg.prevLogIndex > log[log.size() - 1].index || log[arg.prevLogIndex - log[0].index].term != arg.prevLogTerm) {
	return 0; //
    }
...
void raft<state_machine, command>::run_background_apply() {
	...
	if(commitIndex > lastApplied) {
	    ...
	    //std::cout << "typeid = " << typeid(log[start].cmd).name() << std::endl;
	    for(int i = start; i <= end; ++i) {
		//RAFT_LOG("apply a log at index %d\n", i);
		std::cout << "log-i-typeid = " << typeid(log[i].cmd).name() << std::endl;
