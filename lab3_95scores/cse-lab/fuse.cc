...
getattr(chfs_client::inum inum, struct stat &st)
{
    ...
    if(chfs->isdir(inum) == true) {
        chfs_client::dirinfo info;
        ret = chfs->getdir(inum, info);
        if(ret != chfs_client::OK)
            return ret;
        st.st_mode = S_IFDIR | 0777;
        st.st_nlink = 2;//newly created dir has a linked num of 2(. ..).
        st.st_atime = info.atime;
        st.st_mtime = info.mtime;
        st.st_ctime = info.ctime;
        printf("   getattr -> %lu %lu %lu\n", info.atime, info.mtime, info.ctime);
    }
    if(chfs->issymlink(inum) == true) {//this part is newly added by myself, which is used to handle the SYMLINK-attr-get.
    	chfs_client::syminfo sym;
	//ret = chfs->getsymlink(inum, sym);
	ret = chfs->readlink(inum, sym);
	if(ret != chfs_client::OK) return ret;
	st.st_mode = S_IFLNK | 0777;//give all the right to the slink file.
	st.st_nlink = 1;//the slink is a file only having different type, so it's link num is equal to a regular file.
	st.st_atime = sym.atime;
	st.st_mtime = sym.mtime;
	st.st_ctime = sym.ctime;
	st.st_size = sym.size;
    }
        ...
        off_t off, struct fuse_file_info *fi)
{
    ...
    chfs_client::status ret;
    std::string data;
    ret = chfs->read(inum, size, off, data);
    if(ret != chfs_client::OK) {
    	fuse_reply_err(req, ENOENT);
	return ;
    }
...
#if 1
    // Change the above line to "#if 1", and your code goes here
    //printf("	fuseserver_write: ino: %d, size: %d, off: %d\n", ino, size, off);
    chfs_client::inum inum = ino;
    chfs_client::status ret;
    size_t written = 0;
    ret = chfs->write(ino, size, off, buf, written);
    if(ret != chfs_client::OK) {
    	fuse_reply_err(req, ENOENT);
	return ;
    }
        ...
        mode_t mode)
{
    ...
    // Change the above line to "#if 1", and your code goes here
    chfs_client::status ret;
    ret = fuseserver_createhelper(parent, name, mode, &e, extent_protocol::T_DIR);
    if(ret != chfs_client::OK) {
    	if(ret == chfs_client::EXIST) fuse_reply_err(req, EEXIST);
...
fuseserver_readlink(fuse_req_t req, fuse_ino_t ino)
{
	...
	fuse_reply_readlink(req, sym.slink.c_str());
}

//edition 1:implementation is similar to the fuseserver_mkdir() function.
void
fuseserver_createsymlink(fuse_req_t req, const char* link, fuse_ino_t parent, const char* name)
{//NOTE:the arguments' type should be equal to the types given in the FUSE doc.
	struct fuse_entry_param e;
	e.attr_timeout = 0.0;
	e.entry_timeout = 0.0;
	e.generation = 0;
	(void) e;
#if 1
	chfs_client::inum inum1 = parent;
	chfs_client::inum inum2;
	chfs_client::status ret;
	ret = chfs->createsymlink(inum1, name, link, inum2);
	if(ret != chfs_client::OK) {
	    if(ret != chfs_client::EXIST)fuse_reply_err(req, ENOENT);
	    else fuse_reply_err(req, EEXIST);
	    return ;
	}
	getattr(inum2, e.attr);
	e.ino = inum2;
	fuse_reply_entry(req, &e);
#else
	fuse_reply_err(req, ENOSYS);
...
main(int argc, char *argv[])
{
     ...
     * rmdir, etc.
     * */
    //NOTE:Don't forget to add the new mappings between your implementation and the FUSE's here! 
    fuseserver_oper.symlink    = fuseserver_createsymlink;//note that the arguments' sequence is essential,see the web page for details.
