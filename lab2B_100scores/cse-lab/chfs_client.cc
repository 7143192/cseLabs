...
chfs_client::createsymlink(inum parent, const char* name, const char* slink, inum& ino_out)
{
    //lc->acquire(ino_out);
    lc->acquire(parent);
    ec->addBasicLog(chfs_command::CMD_BEGIN, txid++);//add BEGIN here,TODO:handle the txid here!!!! 
    int r = OK;
    //txid++;
    ec->txid = txid;
    bool checked = false;
	...
	//lc->release(ino_out);
	lc->release(parent);
	return EXIST;//the slink has already existed.
    }
    if(ec->create(extent_protocol::T_SYMLINK, ino_out) != OK) {
	//lc->release(ino_out);
	lc->release(parent);
	return IOERR;//allocate a new inode number to the symbolic link file.
    }
    
    ec->addPutLog(txid, ino_out, "", 0, std::string(slink), (std::string(slink)).size());
    ...
    std::string buf1 = buf;//old val of the buf.
    ...
    ec->addPutLog(txid, parent, buf1, buf1.size(), buf, buf.size());
    ...
    ec->addBasicLog(chfs_command::CMD_COMMIT, txid);//add COMMIT log here.TODO:handle the txid here!!!!
...
{//mainly used to reset the size and the content of the file.
    ...
    lc->acquire(ino);
    ec->addBasicLog(chfs_command::CMD_BEGIN, txid++);//TODO:handle the txid here!!!!
    int r = OK;
    /*
     * your code goes here.
    ...
    ec->get(ino, buf);
    if(buf.size() == size) {
	ec->addBasicLog(chfs_command::CMD_COMMIT, txid);//
	lc->release(ino);
        return r;
    }
	...
	uint32_t old = buf.size();//
	std::string old_buf = buf;//
    	buf = buf.substr(0, size);
	ec->addPutLog(txid, ino, old_buf, old, buf, size);//
	ec->put(ino, buf);
	ec->addBasicLog(chfs_command::CMD_COMMIT, txid);//
	lc->release(ino);
	return r;
    ...
    ec->addPutLog(txid, ino, old_buf, old, buf, size);//
    ec->put(ino, buf);
    ec->addBasicLog(chfs_command::CMD_COMMIT, txid);//TODO:handle the txid here!!!
    lc->release(ino);
    return r;
}
...
chfs_client::create(inum parent, const char *name, mode_t mode, inum &ino_out)
{
    ...
    ec->addPutLog(txid, parent, old_buf, old, buf, buf.size());//
...
{//similar to the implementation of the create() function.
	...
	lc->release(parent);
	//lc->release(ino_out);
	return EXIST;//handle the case that the dir has existed.
    }
...
chfs_client::lookup(inum parent, const char *name, bool &found, inum &ino_out)
{
	...
	//lc->release(ino_out);
	//lc->release(parent);
	return r;//if the inode is empty, return directly.
    }
    //lc->release(parent);
    std::string tmp = buf;
    //if(tmp.size() != 0) tmp -= ',';
    ...
    return r;
    /*
    std::list<dirent> dir_info;
    readdir(parent, dir_info);
    if(dir_info.size() == 0) {
        return r;
    }
        ...
        size_t &bytes_written)
{
    ...
    if(off + size <= buf.size()) {
    	bytes_written = size;
	for(uint32_t i = off; i < off + size; ++i) buf[i] = data[i - off];
    }
    else {
    	/*if(off < buf.size()) {
	    //int cur = buf.size();
	...
	for(uint32_t i = 0; i < buf.size(); ++i) tmp[i] = buf[i];
	buf = tmp;
	for(uint32_t i = off; i < off + size; ++i) buf[i] = data[i - off];
	bytes_written = size;
    }

    ec->addPutLog(txid, ino, old_buf, old, buf, buf.size());
...
int chfs_client::unlink(inum parent,const char *name)
{
	...
	ec->addBasicLog(chfs_command::CMD_COMMIT, txid);//
	lc->release(parent);
	return NOENT;//the file is not found in the parent dir, return directly.
    }
    lc->acquire(inum);
