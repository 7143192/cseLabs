...
# run test a
perl ./test-lab2a-part1-a.pl chfs1 | grep -q "Passed all"
if [ $? -ne 0 ];
then
        echo "Failed test-A"
        #exit
...
# run test b
perl ./test-lab2a-part1-b.pl chfs1 | grep -q "Passed all"
if [ $? -ne 0 ];
then
        echo "Failed test-B"
        #exit
...
# run test c
./test-lab2a-part1-c.sh chfs1 | grep -q "Passed SYMLINK"
if [ $? -ne 0 ];
then
        echo "Failed test-C"
        #exit
