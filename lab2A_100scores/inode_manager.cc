...
void
inode_manager::write_file(uint32_t inum, const char *buf, uint32_t size)
{
  /*
   * your code goes here.
  ...
  uint32_t old = (uint32_t)ino->size;
  uint32_t old_filled = old / BLOCK_SIZE;
  uint32_t old_left = old % BLOCK_SIZE;
  uint32_t old_num = (old_left == 0) ? old_filled : (old_filled + 1);//count the old block number needed.
  uint32_t new_filled = size / BLOCK_SIZE;
  uint32_t new_left = size % BLOCK_SIZE;
  uint32_t new_num = (new_left == 0) ? new_filled : (new_filled + 1);//count the new block number needed.
  //int old1 = (old == 0) ? 0 : (old - 1) / BLOCK_SIZE + 1;
  //int new1 = (size == 0) ? 0 : (size - 1) / BLOCK_SIZE + 1;
